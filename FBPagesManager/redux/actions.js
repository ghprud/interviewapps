...
export function checkAppToken() {
  console.log("checking app token from actions.js")
  return dispatch => {
    dispatch(appTokenCheck()) // state is changed to requesting login to true...
    return new Promise( (resolve, reject) => {
      facebookAPI.checkAccessToken()
        .then( (tokenDetails) => {
...
export function pageInfo(pageId) {
  return dispatch => {
    ...
    return new Promise( (resolve, reject) => {
      facebookAPI.pageDetails(pageId)
        .then((pageDetails) => {
...
// for fetching the insights of a post...
...
// function to bundle pageContent and postInsights
...
export function sendSchedulePost(pageId, token, parameters) {
  return dispatch => {
    dispatch(postSend())
    return new Promise( (resolve, reject) => {
      facebookAPI.sendScheduledPost(pageId, token, parameters)
        .then((result) => {
        dispatch(postSendSuccess(result)) // create an other method...
        resolve(result)
      }).catch((error) => {
        dispatch(postSendFailure(error)) // create another method....
        reject(error)
      })
    })
  }
...
export function sendPost(pageId, token, parameters) {
  return dispatch => {
    dispatch(postSend())
    return new Promise( (resolve, reject) => {
      facebookAPI.sendPost(pageId, token, parameters)
        .then((result) => {
